...
import voteIcon from "./assets/24473275.png";
import megaphoneIcon from "./assets/24473275.png";
import checkIcon from "./assets/24473275.png";
import handIcon from "./assets/24473275.png";
import ballotIcon from "./assets/24473275.png";
...
const VotePage = () => {
  ...
  const [user, setUser] = useState({ national_id: 2000600248 });
  ...
  const handleVote = () => {
    ...
    const isConfirmed = window.confirm(
      `هل أنت متأكد من التصويت لحزب "${selectedParty}"؟`
    );

    if (isConfirmed) {
      axios
        .patch("http://localhost:5000/votedparty", {
          user,
        ...
        {hasVoted ? (
          <div className="p-6 mb-6 bg-white mx-10 rounded-lg shadow-lg mt-[10%]">
            <h2 className="text-2xl font-semibold text-gray-800">
              {" "}
              تم تسجيل صوتك للقائمة الحزبية{" "}
            </h2>
            <div className="flex justify-center mt-4">
              <button className="px-6 py-3 rounded-full bg-green-900 text-white hover:bg-green-700 transition duration-300 shadow-lg text-lg font-semibold">
